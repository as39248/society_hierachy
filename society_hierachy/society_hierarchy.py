...
class Citizen:
    ...
    def add_subordinate(self, subordinate: Citizen) -> None:
        ...
        if self._subordinates == []:
            ...
            self._subordinates[0].set_superior(self)
        elif subordinate.cid > self._subordinates[-1].cid:
            self._subordinates.append(subordinate)
            self._subordinates[-1].set_superior(self)
        elif subordinate.cid < self._subordinates[0].cid:
            self._subordinates = [subordinate] + self._subordinates
            self._subordinates[0].set_superior(self)
        else:
            while i < len(self._subordinates):
                if self._subordinates[i].cid < subordinate.cid \
                    ...
                    self._subordinates[i + 1].set_superior(self)
                    break
                i += 1
    ...
    def get_highest_rated_subordinate(self) -> Citizen:
        ...
        for citizen in self.get_direct_subordinates():
            lst.append(citizen.rating)
            c_lst.append(citizen)
        return c_lst[lst.index(max(lst))]
...
class Society:
    ...
    def add_citizen(self, citizen: Citizen, superior_id: int = None) -> None:
        ...
        if superior_id is None:
            if original_head is not None:
                ...
                original_head.become_subordinate_to(self._head)
            else:
                self.set_head(citizen)
        else:
            citizen.become_subordinate_to(self._head.get_citizen(superior_id))
    ...
    def change_citizen_type(self, cid: int,
        ...
        """
        old = self._head.get_citizen(cid)
        old_superior = old.get_superior()
        if isinstance(old, DistrictLeader):
            new = Citizen(old.cid, old.manufacturer,
                          old.model_year, old.job, old.rating)
            if old.get_superior() is not None:
                old.get_superior().remove_subordinate(cid)
                old_superior.add_subordinate(new)
            for subordinates in old.get_direct_subordinates():
                new.add_subordinate(subordinates)
            return new

        else:
            new = DistrictLeader(old.cid, old.manufacturer, old.model_year,
    ...
    def _swap_up(self, citizen: Citizen) -> Citizen:
        ...
        new_l = citizen
        new_c = citizen.get_superior()

        # swap leader
        if isinstance(citizen.get_superior(), DistrictLeader):
            new_l = self.change_citizen_type(citizen.cid,
                                             citizen.get_district_name())
            new_c = self.change_citizen_type(new_l.get_superior().cid)
        ...
        new_l.job = old_l_job
        # swap position
        subs_new_c = new_c.get_direct_subordinates()
        subs_new_l = new_l.get_direct_subordinates()

        if new_c.get_superior() is not None:
            super_sup = new_c.get_superior()
            super_sup.remove_subordinate(new_c.cid)

            for s in subs_new_c:
                new_c.remove_subordinate(s.cid)

            if subs_new_l != []:
                for s in subs_new_l:
                    new_l.remove_subordinate(s.cid)
                    new_c.add_subordinate(s)

            super_sup.add_subordinate(new_l)

            self._helper(subs_new_c, new_l, new_c)
    ...
    def _helper(self, sub: List[Citizen], new_l: Citizen, c: Citizen) -> None:
        """helper function for swap up
        sub: subs_new_c in _swap_up
        new_l: new_l in _swap_up
        c: new_c in _swap_up
        """
        for s in sub:
            if s.cid != new_l.cid:
                new_l.add_subordinate(s)

        new_l.add_subordinate(c)
    ...
    def promote_citizen(self, cid: int) -> None:
        ...
        if citizen.get_superior() is not None and \
                citizen.rating > citizen.get_superior().rating and \
    ...
    def delete_citizen(self, cid: int) -> None:
        ...
        Precondition: There is a Citizen with the cid <cid> in this Society.
        """
        if self._head == self.get_citizen(cid) and \
                self.get_citizen(cid).get_direct_subordinates() != []:
            this = self.get_citizen(cid)
            self._head = self._head.get_highest_rated_subordinate()
            for c in this.get_direct_subordinates():
                if c != self._head:
                    c.become_subordinate_to(self._head)
            self._head.set_superior(None)

        elif self._head == self.get_citizen(cid) and \
                self.get_citizen(cid).get_direct_subordinates() == []:
            self._head = None

        else:
            subordinates = self.get_citizen(cid).get_direct_subordinates()
            sup = self.get_citizen(cid).get_superior()
            self.get_citizen(cid).get_superior().remove_subordinate(cid)
            if subordinates != []:
